...
def drip():
    ...
    if not oset:
        ...
        total_water = sum(v in ('~', '|')
                          for c, v in coords.items() if c[1] >= min_y)
    ...
    while (left not in coords or coords[left] == '|') and down in coords and coords[down] != '|':
        coords[left] = '|'
    ...
    while (right not in coords or coords[right] == '|') and down in coords and coords[down] != '|':
        coords[right] = '|'
        right = (right[0] + 1, right[1])
        down = (right[0], right[1] + 1)

    # contained?
    if (left in coords and right in coords and
            coords[left] == '#' and coords[right] == '#'):
        for i in range(left[0] + 1, right[0]):
            if (i, active[1]) in coords and coords[i, active[1]] == '#':
                raise Exception('BAD')
            coords[(i, active[1])] = '~'
        status.text += 'settled'
...
def main():
    ...
    ftc.text = display()
    root = HSplit([Window(content=ftc),
                   HorizontalLine(),
